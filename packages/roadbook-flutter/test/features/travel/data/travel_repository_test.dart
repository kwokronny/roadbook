...
Map<String, dynamic> _travelJson({int id = 1, String name = 'Trip A'}) => {
  'id': id, 'name': name, 'startDate': '2024-06-01', 'endDate': '2024-06-05',
  'public': false, 'city': '北京', 'Users': [], 'Schedules': [],
};
...
void main() {
  group('TravelRepository', () {
    ...
    test('page hasMore is false when all loaded', () async {
      dio.interceptors.add(InterceptorsWrapper(onRequest: (options, handler) {
        handler.resolve(Response(requestOptions: options, statusCode: 200, data: {
          'record': [_travelJson()], 'total': 1, 'page': 1, 'pageSize': 15,
        }));
      }));
      final result = await repo.page(page: 1, keyword: '');
    ...
    test('save sends correct payload and returns Travel', () async {
      ...
      final travel = await repo.save(TravelFormData(
        name: 'New Trip', startDate: DateTime(2024, 9, 1), endDate: DateTime(2024, 9, 5),
        isPublic: false, cities: ['深圳', '广州'],
      ));
      expect(travel.id, 99);
    });
    ...
    test('save includes id when editing existing travel', () async {
      ...
      await repo.save(TravelFormData(
        id: 5, name: 'Edit Trip', startDate: DateTime(2024, 9, 1),
        endDate: DateTime(2024, 9, 3), isPublic: false, cities: [],
      ));
      expect(capturedId, 5);
    });
